...
```{r import data}
local_file = "Weekly total-Table 1.csv"
Weekly_total <- read.csv("local_file", header = T)
...
local_file2 <- "Monthly average-KetSDA.csv"
Monthly_average <- read.csv("local_file2", header = T)
...
```{r Future}
future <- subset(Weekly_total, Date > as.Date("2021-04-01"))
future <- future %>% 
        group_by(month = floor_date(Date, "month")) %>% 
        summarize(attendance = mean(attendance, na.rm = TRUE))
future.ts <- ts(future$attendance, start = c(2021, 4), frequency = 12)
```
...
```
The five models I tested were SARIMA, Prophet, GLMNet, Random Forest, and Prophet with XGBoost errors. The last three are all machine learning algorithms. All five were fitted using the training time period and then attempted to predict the testing time period.
...
compare <- calibration_table %>%
        ...
        modeltime_refit(monthly) %>%
        modeltime_forecast(h = "36 months", actual_data = monthly) %>%
        filter(.index >= "2021-04-01" & .index <= "2021-10-01") %>%
        select(.index, .value) %>%
        rename(Month = .index) %>%
...
ggplot(pred_vs_actual, aes(x = Month, y = value, color = Category)) +
        ...
        labs(title = "Predicted vs actual in-person attendance",
             subtitle = "April 2021 - September 2021",
             x = "Month",
             y = "Attendance")
```
...
month <- c("2021-04-01", "2021-05-01", "2021-06-01", "2021-07-01", "2021-08-01", "2021-09-01", "2021-10-01")
month <- as.Date(month)
...
In-person attendance has ranged from `r max(percent_diff)`% lower than predicted in May 2021 down to `r min(percent_diff)`% lower than predicted in August 2021.
...
p <- ggplot(Full_Average_by_month, aes(x = Date, y = attendance, color = color)) +
        theme_bw() +
        geom_line() +
        geom_point() +
        geom_smooth(method = "loess", formula = "y~x", col = "red") +
        labs(title = "Weekly mean combined attendance by month 2002-2021",
             subtitle = "Kettering SDA Church, Kettering, Ohio",
             x = "Date",
             y = "Average weekly combined attendance") +
        scale_fill_continuous(guide = guide_legend()) + 
        theme(legend.position = "bottom")
p
```
```{r, contrast total vs in-person}
g
```
...
I created a sample of the type of survey I would like to use to gather this information. You can find it on my Shiny Apps page at https://jrmilks.shinyapps.io/KetSDA_demographics/ Note that this is just an example. I'm sure there are more sophisticated ways to gather this information.
