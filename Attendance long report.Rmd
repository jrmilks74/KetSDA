...
date: "Last compiled on `r format(Sys.Date(), '%d %B, %d')`"
output:
...
Combined_Average_by_month <- Weekly_total %>% 
        group_by(month = floor_date(Date, "month")) %>% 
        summarize(Combined = mean(Combined, na.rm = TRUE))

p <- ggplot(Combined_Average_by_month, aes(x = month, y = Combined)) +
        theme_bw() +
        geom_line() +
        geom_point() +
        geom_smooth(method = "loess", formula = "y~x", col = "red") +
