...
combined <- rbind(Monthly_average_sub, combined) %>%
        mutate(color = ifelse(Date < as.Date("2017-12-01"),
                              "In-person",
                              "In-person + Online")
               )
...
Ascent_monthly_average <- Ascent %>%
        group_by(month = floor_date(Date, "month")) %>%
        summarize(in.person = mean(First, na.rm = TRUE),
                  online = mean(FirstServe24, na.rm = TRUE),
...
Sanctuary_monthly_average <- Sanctuary %>%
        group_by(month = floor_date(Date, "month")) %>%
        summarize(in.person = mean(Second, na.rm = TRUE),
                  online = mean(SecondServe24, na.rm = TRUE),
                  total = mean(total, na.rm = TRUE)
                  )
...
Sanctuary_overview_table <- matrix(c(tail(Sanctuary$total, 1),
                                     tail(Sanctuary$Second, 1),
                                     tail(Sanctuary$SecondServe24, 1),
